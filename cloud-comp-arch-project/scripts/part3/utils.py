...
MCPATH = "/home/ubuntu/memcache-perf-dynamic/mcperf"


def make_exp_dir():
    output_path = f"logs/part3/{datetime.now().strftime("%Y_%m_%d_%H_%M_%S")}/"
...
def run_remote_command(remote_ip, command_line, block=True):
    ...
    if block:
        ...
        return Popen(args, stdout=subprocess.PIPE, text=True)
